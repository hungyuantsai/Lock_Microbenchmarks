...
mcsrw::MCSRWLock rw;
mcsrw::MCSRWQNode qnode;

void reader_thread(mcsrw::MCSRWQNode *I) {
...
void writer_thread(mcsrw::MCSRWQNode *I) { 
...
void *thread(void *) {
    unsigned int num = thread_counter.fetch_add(1, std::memory_order_release) % Num_Core;
	cpu_set_t cpuset;
	CPU_ZERO(&cpuset);
	CPU_SET(num, &cpuset);
